...
public class JTextFieldPorcentagem extends JTextField {
    private static final NumberFormat MONETARY_FORMAT = new DecimalFormat("#,##0.00");
    private NumberFormat numberFormat;
    private int limit = -1;

    public JTextFieldPorcentagem(int casasDecimais) {
        this(new DecimalFormat((casasDecimais == 0 ? "#,##0" : "#,##0.") + makeZeros(casasDecimais)));
    }

    public JTextFieldPorcentagem() {
        this(MONETARY_FORMAT);
    }

    public JTextFieldPorcentagem(NumberFormat format) {
        numberFormat = format;
        ...
        setDocument(new PlainDocument() {
            ...
            public void insertString(int offs, String str, AttributeSet a) throws BadLocationException {
                String text = new StringBuilder(JTextFieldPorcentagem.this.getText().replaceAll("[^0-9]", VAZIO)).append(str.replaceAll("[^0-9]", VAZIO)).toString();
