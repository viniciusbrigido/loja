...
public class CadastroProdutoController extends CadastroController {
    ...
    private void verificaMarcaTipoTamanhoCorCadastrados() {
        if (getMarcas().isEmpty()) {
            showMessageDialog(getView(), "N�o h� Marcas cadastradas.", "Aten��o", INFORMATION_MESSAGE);
            getView().dispose();
            return;
        }
        if (getTipos().isEmpty()) {
            showMessageDialog(getView(), "N�o h� Tipos de Produto cadastrados.", "Aten��o", INFORMATION_MESSAGE);
            getView().dispose();
            return;
        }
        if (getTamanhos().isEmpty()) {
            showMessageDialog(getView(), "N�o h� Tamanhos cadastrados.", "Aten��o", INFORMATION_MESSAGE);
            getView().dispose();
            return;
        }
        if (getCores().isEmpty()) {
            showMessageDialog(getView(), "N�o h� Cores cadastradas.", "Aten��o", INFORMATION_MESSAGE);
            getView().dispose();
        }
    }
    ...
    public void buscaProduto() {
        ...
        if (isNotNull(produto) && isNotEmpty(produto.getId())) {
            ...
            showMessageDialog(getView(), "Produto n�o encontrado.", "Aten��o", ERROR_MESSAGE);
            limpaTela();
        }
    }
    ...
    private void verificaProdutoJaCadastrado() throws Exception {
        ...
        showMessageDialog(getView(), "C�digo n�o encontrado.\nPara cadastrar um novo Produto n�o preencha o campo de c�digo.", "Aten��o", ERROR_MESSAGE);
    ...
    public void listaItens() {
        ...
        if (getProdutos().isEmpty()) {
            showMessageDialog(getView(), "N�o h� Produtos para listar.", "Aten��o", INFORMATION_MESSAGE);
            return;
        }
    ...
    private List<ColunaDto> getColunas() {
        ...
        colunas.add(new ColunaDto("Descri��o", ESQUERDA, 90));
    ...
    public void salvaCaracteristica() {
        ...
        try {
            String msg = format("Caracter�stica do Produto %s com sucesso.", isEmpty(codigoCaracteristica) ? "cadastrada" : "alterada");
